...
struct StructLayout { std::unordered_map<std::string, int> fieldOffsets; int size = 0; };

class CodegenASM { std::vectorstd::string asmLines; int labelCount = 0; std::stackstd::string breakLabels; std::stackstd::string continueLabels; std::unordered_map<std::string, StructLayout> structLayouts; std::unordered_map<std::string, int> localOffsets; int currentOffset = 0;
...
public: std::string generate(std::shared_ptr<ProgramNode> program) { asmLines.clear(); emit(".text"); emit(".global _start"); emit("_start:"); emit("  bl main"); emit("  mov x8, #93"); emit("  mov x0, #0"); emit("  svc #0");
...
void genExpr(const NodePtr& expr) {
    switch (expr->kind) {
        case NodeKind::Literal: {
            ...
            if (isdigit(lit->value[0])) {
                emit("  mov x0, #" + lit->value);
